...
import { BookInfo, Status } from "../type-defining/type.js";

export const confirmBook = document.getElementById(
  "confirm-book"
) as HTMLDivElement;
...
export let bookedInfo: BookInfo[] = [];

const localBookData = localStorage.getItem("bookedInfo");
bookedInfo = localBookData ? JSON.parse(localBookData) : [];

const saveBookings = (): void => {
  localStorage.setItem("bookedInfo", JSON.stringify(bookedInfo));
};

const findPendingBooking = (currentUser: string): BookInfo | undefined => {
  return bookedInfo.find(
    (book) => book.user === currentUser && book.status === Status.Pending
  );
};

export const setBookingData = (date: string): void => {
  const currentUser: string | undefined = getToken("username");
  if (!currentUser) return;

  const pending = findPendingBooking(currentUser);
  if (pending) return alert("You have a pending booking");
...
const renderBookingInfo = (pending: BookInfo): void => {
  confirmBook.innerHTML = `
...
const handleConfirm = (pending: BookInfo) => {
  {
...
const startBookingTimer = (pending: BookInfo) => {
  const timer = document.getElementById("timer") as HTMLParagraphElement;

  if (intervalId) clearInterval(intervalId);

  intervalId = setInterval(() => {
    const remaining: number = Math.ceil(
      (pending.createdAt + BOOKING_EXPIRY_MS - Date.now()) / 1000
    );
