...
admnis.map(function (admin) {
    var activity = admin.isActive; //infer type boolean and string
    console.log(activity);
});
//Here type define is not understand
var groupBy = admnis.reduce(function (acc, cur) {
    var roleName = cur.role.name;
    if (!acc[roleName]) {
        acc[roleName] = [];
    }
